...
The leaderboard no longer contains sample or automatically generated players. It stays empty until verified on-chain chess activity is recorded. The current browser chessboard does not write every move to the blockchain; a production deployment needs a secure backend or smart contract plus an indexer to record completed games, wins, scores, and weekly prize accounting.
